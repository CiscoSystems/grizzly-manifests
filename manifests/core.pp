  ...
  if ($osfamily == 'debian') {
    ...
    if($::package_repo == 'cisco_repo') {
      apt::source { 'cisco-openstack-mirror_grizzly':
        location    => "$::location/dfa",
        release     => 'grizzly-proposed',
        repos       => 'main',
        key         => '8FC026DB0AC59B1A',
...
class control(
  ...
  if $enable_ha {
    ...
    class { 'openstack-ha::controller':
      ...
      rabbit_password          => $rabbit_password,
      rabbit_user              => $rabbit_user,
      rabbit_hosts             => ["${controller01_hostname}:5672","${controller02_hostname}:5672","${controller03_hostname}:5672"],
    ...
    class { 'openstack::controller':
      ...
      glance_user_password    => $glance_user_password,
      glance_backend          => $glance_backend,
      glance_rbd_store_user   => $rbd_store_user,
      glance_rbd_store_pool   => $rbd_store_pool,
      nova_db_password        => $nova_db_password,
      nova_user_password      => $nova_user_password,
      rabbit_password         => $rabbit_password,
      rabbit_user             => $rabbit_user,
